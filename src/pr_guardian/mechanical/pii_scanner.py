...
PII_LOG_PATTERNS = [
    ...
    (
        re.compile(
            r"(?:log|logger|console\.log|print)\s*[.(].*(?:user\.name|customer\.|patient\.)", re.I
        ),
        "PII entity in log statement",
        CheckSeverity.WARNING,
    ),
]
...
TEST_PII_PATTERNS = [
    (
        re.compile(r"[\w.+-]+@[\w-]+\.[\w.-]+"),
        "real-looking email in test data",
        CheckSeverity.WARNING,
    ),
