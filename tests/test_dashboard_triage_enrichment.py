    ...
    return {
        ...
        "agent_results": [
            {"agent_name": "security", "findings": [
                {"id": str(uuid.uuid4()), "severity": "high",   "certainty": "detected",  "file": "a.py", "category": "x"},
                {"id": str(uuid.uuid4()), "severity": "medium", "certainty": "uncertain", "file": "b.py", "category": "y"},
                {"id": str(uuid.uuid4()), "severity": "low",    "certainty": "detected",  "file": "c.py", "category": "z"},
                {"id": str(uuid.uuid4()), "severity": "low",    "certainty": "uncertain", "file": "d.py", "category": "w"},
            ]},
        ],
    }
...
def _patch(monkeypatch, review):
    ...
    async def _active(*_a, **_k): return []
    async def _archived(*_a, **_k): return []
...
def test_triage_counts_rollup_matches_per_finding_classes(client, fake_review_with_mixed_findings, monkeypatch):
    _patch(monkeypatch, fake_review_with_mixed_findings)
...
def test_endpoint_still_works_when_dismissal_lookup_fails(client, fake_review_with_mixed_findings, monkeypatch):
    ...
    async def _get(_id): return fake_review_with_mixed_findings
    async def _boom(*_a, **_k): raise RuntimeError("db down")
